    ...
    if (allHeld && allSameValue) {
      ...
      setTimeSpent((prevTimeSpent) => {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            time:
              oldTimeSpent === null || newTimeSpent < oldTimeSpent
                ? newTimeSpent
                : oldTimeSpent,
            rolls: oldRolls === null || rolls < oldRolls ? rolls : oldRolls,
          })
        );
        return newTimeSpent;
      });
  ...
  return (
    <main>
      {tenzies && (
        <Confetti
          width={width - 30}
          colors={["#66B53A", "#ECF65A", "#F65A96", "#B55CE3", "#5CCCE3"]}
        />
      )}
      ...
      <div className="buttons">
        ...
        {!tenzies && <button className="roll-dice" onClick={restartGame}>
          Restart
        </button>
        }
      </div>
      {tenzies && (
        <p className="record-time">
          Your record time spent:{" "}
          {Math.floor(getTimeFromLocalStorage() / 1000 / 60)}m{" "}
          {Math.floor(getTimeFromLocalStorage() / 1000) % 60}s{" "}
          {Math.floor(getTimeFromLocalStorage() % 1000)}ms. Your record number
          of rolls spent: {getRollsFromLocalStorage()}.
        </p>
      )}
    </main>
  );
